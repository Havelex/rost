...
pub fn init_paging(
    ...
    //    Limine sets up HHDM in its own page tables before running the kernel,
    //    so hhdm_offset + phys is valid immediately (identity mapping is NOT
    //    available under Limine's boot environment).
